...
class TestAssertion:
    ...
    def test_rcv_assorter(self):
        import json
        with open('./shangrla/shangrla/tests/Data/334_361_vbm.json') as fid:
            data = json.load(fid)
    ...
    def test_set_pool_means(self):
        ...
        #
        # without use_style
        self.raw_AvB_asrtn.assorter.set_pool_means(cvr_list=cvr_list, tally_pool=pool, use_style=False)
        ...
        self.raw_AvB_asrtn.assorter.set_pool_means(cvr_list=cvr_list, pool=pool, use_style=True)
        np.testing.assert_almost_equal(self.raw_AvB_asrtn.assorter.pool_means['1'], (1+1/2+1/2)/3)
        np.testing.assert_almost_equal(self.raw_AvB_asrtn.assorter.pool_means['2'], (0+1/2)/2)
        #
